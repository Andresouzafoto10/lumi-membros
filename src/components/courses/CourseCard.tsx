...
import type { CourseAccess, CourseCardOrientation } from "@/types/course";
...
interface CourseCardProps {
  ...
  /** Visual banner format used by the parent session */
...
export const CourseCard = memo(function CourseCard({
  ...
  const cardContent = (
    <Card
      className={cn(
        "overflow-hidden border border-border/50 shadow-md transition-all duration-300 group-hover:shadow-xl group-hover:shadow-primary/5 group-hover:-translate-y-1 group-hover:border-primary/20",
        isVertical && "bg-transparent shadow-none group-hover:shadow-primary/10"
      )}
    >
      <div className="relative">
        <AspectRatio ratio={isVertical ? 9 / 16 : 16 / 9}>
          ...
          <div
            className={cn(
              ...
              isVertical && "from-black/75 via-black/10"
            )}
          />
          {isUpcoming && (
        ...
        {isVertical && isUpcoming && courseId && (
          <Button
            size="sm"
            variant={interested ? "default" : "outline"}
            className="absolute inset-x-3 bottom-3 z-10 gap-2 bg-background/90 backdrop-blur hover:bg-background"
        ...
        progressPercent != null && !locked && !isUpcoming ? (
          <div className="px-1.5 py-1.5">
            <Progress
              ...
              className="h-[3px] bg-muted/80"
              indicatorClassName="bg-primary"
              aria-label={`Progresso de ${title}: ${Math.round(progressPercent)}%`}
            />
          </div>
        ) : null
      ) : (
