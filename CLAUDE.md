...
- **MCP Supabase tools estão conectados e funcionando** neste projeto (`gdbkbeurjjtjgmrmfngk`). Use as ferramentas MCP (`mcp__supabase__*`) para DDL/SQL, listagem de tabelas, execução de queries, etc. Fallback: SQL Editor no dashboard ou `curl` com a service role key do `.env` (`VITE_SUPABASE_SERVICE_ROLE_KEY`).
...
- **Cloudflare R2** (exclusive) — Bucket `lumi-membros`. Frontend uploads go through `src/hooks/useR2Upload.ts`, which first calls the Supabase Edge Function `r2-presigned` and then performs a direct `PUT` to the presigned URL. Public asset base stays in `VITE_R2_PUBLIC_URL`. CORS-sensitive rendering paths use `src/lib/imageProxy.ts` and the Edge Function `image-proxy`. Folders: `banners/`, `courses/banners/`, `materials/`, `certificates/backgrounds/`, `communities/covers/`, `communities/icons/`, `profiles/avatars/`, `profiles/covers/`, `posts/images/`, `posts/attachments/`, `legacy/<bucket>/...` (one-off archive of orphan files migrated out of Supabase Storage on 2026-05-13). Images auto-optimized to WebP before upload. Old files deleted on replacement. CORS configured in `r2-cors.json`.
- **Supabase Storage** — Not used. Buckets, objects, and RLS policies were removed on 2026-05-13 (migration `002_drop_storage_policies.sql` + `scripts/delete-supabase-storage.mjs`). Backup manifest of the 6 archived orphans lives at `scripts/migration-manifest.json`.
...
- **`download-material`** — Secure PDF download with social DRM watermarking. Validates JWT, checks enrollment (admin bypass), fetches the file from R2 (`material.file_path` must be an `https://` R2 URL), and injects watermark (name + email + CPF) on PDF pages via pdf-lib. For non-PDF or non-DRM files, returns a 302 redirect to the R2 public URL directly. Rejects any `file_path` not formatted as an HTTPS URL.
